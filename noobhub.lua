...
noobhub = {
	...
	new = function (params) -- constructor method
		...
		function self:create_game(params)
			-- body
			self.game_id = params.game_id or 'test_game_id'
			self.callback = params.callback or   function() end
			self.sock, error_message = socket.connect(self.server,  self.port)
			if (self.sock == nil) then
				print("Noobhub connection error: "..error_message)
		...
		function self:move(message)
				-- TODO: add retries
				if (self.sock == nil) then
					print "NoobHub: Attempt to move without valid subscription (bad socket)"
					self:reconnect()
					return false;
				end
				message.action = 'move';
